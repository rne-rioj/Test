<?php
	require_once $REPORT_COMMISSION;
	$conn = mysqlConn($_SESSION[$DB]);

	$sql = "SELECT * FROM `tags` WHERE `subproject`" . (isset($_SESSION[$PRJ_SUB]) ? " = " .$_SESSION[$PRJ_SUB] : " LIKE '%'"); 
	
	if ( isset($_SESSION[$SEARCH]) ) {
		$sql .= " AND (`name` LIKE '%" . $_SESSION[$SEARCH] . "%' OR `description` LIKE '%" . $_SESSION[$SEARCH] . "%')";
	}	
	
	if ( isset($_SESSION[$TAGS_FILTER]) ) {
		$sql .= $_SESSION[$TAGS_FILTER];
	}
	
	$result = $conn->query($sql);
	$conn->close();
?>
<div class="jumbotron device-info-jumbotron" style="width:100%; padding:0" id="grdWithScroll" onscroll="SetDivPosition()">
	<div class="container" style="width:100%">
		<center>
			<h1><?php echo strtoupper($_SESSION[$PRJ] . (isset($_SESSION[$PRJ_SUB]) ? "-" . $_SESSION[$PRJ_SUB] : "")); ?><br /><small><?php echo $_SESSION[$PRJ_DESC]; ?></small></h1>
			<br />
		</center>
		...
		<div style="width:100%">
			...
			<table id="table-custom-sort" class="table table-bordered table-hover table-condensed sortable" style="width:100%">
				<thead>
					<tr>
				...
				<tbody>
					...
					?>
							<tr class="open-tag-model <?php echo ($row['deleted'] <> 0 ? 'danger' : ($row['completed'] <> 0 ? 'success' : 'warning' )); ?> small" style="cursor: pointer" 
							data-toggle="modal" 
							data-target="#tag-model"
							<?php echo "			
								data-id='".$row["id"]."' 
								data-name='".$row["name"]."' 
							...
							?>
								...
								<td><?php echo strtoupper($_SESSION[$PRJ] . ($row['subproject'] ? "-" . $row['subproject'] : "")); ?></td>
